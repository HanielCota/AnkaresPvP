...
/**
 * CustomInventory provides an easy way to create and manage custom inventories with event handlers.
 */
public class CustomInventory implements InventoryHolder {
    ...
    public CustomInventory(Function<InventoryHolder, Inventory> inventoryFunction) {
        Objects.requireNonNull(inventoryFunction, "inventoryFunction");
        Inventory inv = inventoryFunction.apply(this);

        if (inv.getHolder() != this) {
            throw new IllegalStateException("Inventory holder is not CustomInventory, found: " + inv.getHolder());
        }
    ...
    /**
     * Add an {@link ItemStack} to the inventory on a specific slot with a click handler.
     *
     * @param slot    The slot where to add the item.
     * @param item    The item to add.
     * @param handler The click handler for the item.
    ...
    /**
     * Remove multiple {@link ItemStack}s from the inventory.
     *
    ...
    /**
     * Open the inventory for an InventoryHolder (e.g., Player).
     *
     * @param holder The InventoryHolder to open the menu.
     */
    public void open(InventoryHolder holder) {
        if (holder instanceof Player) {
            ((Player) holder).openInventory(this.inventory);
        } else {
            throw new IllegalArgumentException("Holder must be a Player or subclass of InventoryHolder.");
        }
    }
